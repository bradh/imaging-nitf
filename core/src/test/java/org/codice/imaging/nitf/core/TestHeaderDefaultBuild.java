...
public class TestHeaderDefaultBuild {
    ...
    public void checkNsif10() {
        Nitf nitf = Nitf.getDefault(FileType.NSIF_ONE_ZERO);
        assertNotNull(nitf);
        assertEquals(FileType.NSIF_ONE_ZERO, nitf.getFileType());
        checkEmptyHeader(nitf);
    }

    private void checkEmptyHeader(Nitf nitf) {
        ...
        assertFalse(nitf.getFileDateTime().toZonedDateTime().isAfter(now));
        assertTrue(nitf.getFileDateTime().toZonedDateTime().plusMinutes(1).isAfter(now));
